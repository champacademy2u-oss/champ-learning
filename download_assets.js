...
// Regex or statically grabbed matches from the python script:
const assetUrls = [
...
// Let's also read index.js and index.css to find if we missed other assets.
const jsContent = fs.readFileSync('index.js', 'utf-8');
const cssContent = fs.readFileSync('index.css', 'utf-8');

const allUrls = new Set(assetUrls);

// Search image references in js and css content
const matches = [];
...
async function download(url) {
  ...
  // Use curl as it is very reliable on this OS (mac)
  try {
